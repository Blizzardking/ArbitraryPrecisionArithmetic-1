...
public class ArbitraryPrecisionArithmeticBigBase {
...
class ArbitraryNumber implements Comparable<ArbitraryNumber> {
	...
	private final long BIG_NUMBER_BASE = 1000000000L;
	private final int DIGIT_NUMBER_EACH_NODE = 9;
	...
	public ArbitraryNumber substract(ArbitraryNumber n) throws Exception {
		ArbitraryNumber n3 = new ArbitraryNumber();
		Node p1 = this.header.next;
		Node p2 = n.header.next;
		long borrow = 0;
	...
	public ArbitraryNumber power(ArbitraryNumber n) throws Exception {
		...
		if(n.compareTo(maxLong)>0)
		{
			...
			long expo = Long.parseLong(n.toString());
			return power(this, expo); 
		}
			
	}

	public ArbitraryNumber power(ArbitraryNumber n, long p) throws Exception{
		if(p==0) {
			return new ArbitraryNumber("1");
		}
		if(p%2==0) {
			ArbitraryNumber n1 = n.multiply(n);
			long p1 = p/2;
			ArbitraryNumber n2 = power(n1, p1);
			return n2;
		}
		else {
			ArbitraryNumber n1 = n.multiply(n);
			long p1 = p/2;
			ArbitraryNumber n2 = power(n1, p1).multiply(n);
			return n2;
		}
	}
	...
	public int compareTo(ArbitraryNumber n) {
		ArbitraryNumber n1= new ArbitraryNumber();
		try{
	...
	public ArbitraryNumber append(long b) {
	...
	private class Node {
		/*using long type to store DIGIT_NUMBER_EACH_NODE digits of arbitrary number, Base: 1 billion 
